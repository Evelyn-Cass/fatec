...
db.internacoes.insertOne(
    {
        "paciente_id": 1, "medico_id": 1, "data_internacao":
            ISODate("2023-10-01"), "data_alta": ISODate("2023-10-10"), "motivo":
            "Tratamento de Diabetes"
    });
...
db.internacoes.aggregate([
    ...
    {$unwind: "$medicos"},
    {
        $group:
        {
            _id: "$medicos.especialidade",
            media_idade_pacientes: { $avg: "$pacientes.idade" }
        }
    }
]);
...
db.medicos.aggregate([
    ...
    { $unwind: "$internacoes" },
    {
        $lookup: {
            from: "pacientes",
            localField: "internacoes.paciente_id",
            foreignField: "_id",
            as: "pacientes"
        }
    },
    ...
    {
        $match: {
            "pacientes.diagnóstico": "Diabetes" 
        }
    },
    {
        $group: {
            _id: "$nome", 
            pacientes: { $addToSet: "$pacientes.nome" } 
        }
    }
])
...
db.internacoes.aggregate([
    ...
    },
    {
        $group: {
            _id: "$paciente_id",
            duracao_total_internacao: { $sum: "$duracao_internacao" }
        }
    }
])
