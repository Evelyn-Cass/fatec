    ...
    },
    {
        $project: {
            ...
            crescimento_percentual: {
                ...
                $multiply: [
                    { $divide: [{ $subtract: ["$total_valor", "$previous_total"] }, "$previous_total"] },
                ]
            }
        }
    }
...
db.clientes.aggregate([
    ...
    },
    {
        $group: {
            _id: "$_id",
            nome: { $first: "$nome" },
            total_gasto: { $sum: "$vendas_info.valor" }
        }
    },
    {
        $sort: {
...
db.pedidos.aggregate([
    ...
    {
        $group: {
            _id: { produto: "$produto", regiao: "$clientes_info.regiao" },
            produto: { $first: "$produto" },
            regiao: { $first: "$clientes_info.regiao" },
            total_pedidos: { $sum: 1 }
        }
    },
    {
